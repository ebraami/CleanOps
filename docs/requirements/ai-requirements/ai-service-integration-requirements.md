...
2. At least one `IMAGES` row with `image_type = 'ORIGINAL'` exists for the report and the file is confirmed uploaded to Object Storage.
...
```json
{
  "report_id": 1042,
  "image_id": 2087,
  "image_url": "https://storage.example.com/reports/1042/original.jpg?sig=...&exp=..."
}
```
...
| Q7 | Do we send one image per report or also completion (`AFTER`) images to the AI? | Original images only. |
